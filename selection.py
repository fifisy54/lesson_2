...
def objective(trial):
    ...
    viborka = trial.suggest_int('viborka', 500, 60000)
    ...
    history = model.fit(x_train_new[:viborka], y_train_new[:viborka], batch_size=train_batch_size, epochs=epochs,
                        validation_data=(x_val, y_val), verbose=True)

    scores = model.evaluate(x_test, y_test, verbose=1)
    model.save_weights(f'output/{choice}_{viborka}_{train_batch_size}_{epochs}_{learning_rate}_{viborka}_{round(scores[1] * 100, 4)}.h5')

    if os.path.isfile(f'results.csv') is False:
        df = str(round(scores[1] * 100, 4))
        csv = pd.DataFrame({'NN_model': [choice],
                            'training_set_size': [viborka],
...
if __name__ == "__main__":
    ...
    plotly.offline.plot('Optuna_hypImportances.html')
